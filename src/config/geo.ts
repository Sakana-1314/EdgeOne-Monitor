/**
 * src/config/geo.ts —— 国家/省份编码中文名映射 + 地图名称对齐
...
export const countryName: Record<string, string> = {
  ...
  DZ: '阿尔及利亚', MA: '摩洛哥', TN: '突尼斯', LT: '立陶宛', BG: '保加利亚', SK: '斯洛伐克',
  ...
  BO: '玻利维亚', GY: '圭亚那', SR: '苏里南', PG: '巴布亚新几内亚', FJ: '斐济'
};

export const provinceName: Record<string, string> = {
...
export const resolveCountry = (key: string): string => countryName[key] || key;
export const resolveProvince = (key: string): string => provinceName[key] || key;
...
const EN_TO_ZH: Record<string, string> = {
  ...
  Myanmar: '缅甸', Iran: '伊朗', Iraq: '伊拉克', Afghanistan: '阿富汗',
...
export function buildMapNameMap(): Record<string, string> {
...
export const worldNameMap: Record<string, string> = buildMapNameMap();

/** 把编码 Key 转为地图数据点名称（country: 返回中文名以便与 nameMap 对齐） */
export function toCountryMapName(key: string): string {
